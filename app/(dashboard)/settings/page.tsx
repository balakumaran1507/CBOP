...
type SettingsTab =
  ...
  | 'signatures' | 'email-templates' | 'doc-templates' | 'automation' | 'roles'
...
const DOC_TEMPLATE_VARS = [
  '{{client_name}}', '{{client_org}}', '{{company_name}}', '{{date}}',
  '{{amount}}', '{{role_title}}', '{{scope_note}}', '{{due_date}}',
]
...
function DocTemplatesTab() {
  ...
  const { data, isLoading } = useQuery<{ templates: TemplateItem[] }>({
    queryKey: ['settings-doc-templates'],
    queryFn: () =>
      fetch('/api/settings/templates?output_type=pdf', { credentials: 'include' }).then((r) => r.json()),
  })
  const { data: coData } = useQuery<{ companies: { id: string; name: string }[] }>({
    queryKey: ['settings-companies'],
    queryFn: () => fetch('/api/settings/companies', { credentials: 'include' }).then((r) => r.json()),
  ...
  const all       = data?.templates ?? []
  const templates = filter
    ? all.filter((t) => t.name.toLowerCase().includes(filter.toLowerCase()))
    : all
  const companies = coData?.companies ?? []
  ...
  const deleteMut = useMutation({
    mutationFn: (id: string) =>
      fetch(`/api/settings/templates/${id}`, { method: 'DELETE', credentials: 'include' }).then((r) => r.json()),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['settings-doc-templates'] }); setDeleteId(null) },
  })
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 gap-3">
        <p className="text-sm" style={{ color: 'var(--text2)' }}>
          Document templates for contracts, proposals, certificates and reports.
          For rich editing with the Tiptap editor, use the <a href="/templates" className="underline" style={{ color: 'var(--blue)' }}>Templates page</a>.
        </p>
        <div className="flex items-center gap-2 flex-shrink-0">
          <input
            value={filter}
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-center py-12" style={{ color: 'var(--text3)' }}>
          {filter ? 'No templates match your filter.' : 'No document templates yet.'}
        </p>
      ) : (
        <div className="rounded overflow-hidden" style={{ border: '1px solid #D5DBDB' }}>
          <table className="w-full text-sm">
            <thead>
              <tr style={{ backgroundColor: '#F8F9FA', borderBottom: '1px solid #D5DBDB' }}>
                ...
                <th className="text-left px-4 py-2 font-medium" style={{ color: 'var(--text2)' }}>Category</th>
                <th className="text-left px-4 py-2 font-medium" style={{ color: 'var(--text2)' }}>Company</th>
                <th className="text-left px-4 py-2 font-medium" style={{ color: 'var(--text2)' }}>Ver.</th>
                <th className="text-left px-4 py-2 font-medium" style={{ color: 'var(--text2)' }}>Updated</th>
                <th />
              </tr>
            </thead>
            <tbody>
              {templates.map((t, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-xs" style={{ color: 'var(--text2)' }}>{t.type || '—'}</td>
                  <td className="px-4 py-3">
                    {t.category && (
                      <span className="text-xs px-2 py-0.5 rounded" style={{ backgroundColor: '#E8820C18', color: '#E8820C' }}>
                        {t.category}
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-xs" style={{ color: 'var(--text2)' }}>{t.company_name}</td>
                  <td className="px-4 py-3 text-xs" style={{ color: 'var(--text3)', fontFamily: 'var(--font-mono)' }}>v{t.version}</td>
                  <td className="px-4 py-3 text-xs" style={{ color: 'var(--text3)', fontFamily: 'var(--font-mono)' }}>
                    {fmtDate(t.updated_at)}
                  </td>
                  <td className="px-4 py-3">
      ...
      <DocTemplateSlideOver
        open={soOpen || !!editing}
        template={editing}
        companies={companies}
        onClose={() => { setSoOpen(false); setEditing(null) }}
        onSuccess={() => {
          qc.invalidateQueries({ queryKey: ['settings-doc-templates'] })
          setSoOpen(false)
          setEditing(null)
        }}
      />
...
function DocTemplateSlideOver({
  ...
  const bodyRef = useRef<HTMLTextAreaElement>(null)
  const [form, setForm] = useState({
    name: '', company_id: '', type: 'contract', category: '', content: '', variables: [] as string[],
  })
  const [preview, setPreview] = useState(false)
  const [err, setErr]         = useState('')
  ...
  useEffect(() => {
    if (template) {
      ...
      setForm({ name: '', company_id: companies[0]?.id ?? '', type: 'contract', category: 'contract', content: '', variables: [] })
    }
    setPreview(false)
    setErr('')
  }, [template, companies])
  ...
  const mutation = useMutation({
    mutationFn: (data: typeof form) => {
      ...
      return fetch('/api/settings/templates', {
        ...
        credentials: 'include',
        body: JSON.stringify({
          name: data.name, company_id: data.company_id, output_type: 'pdf',
  ...
  return (
    <SlideOver
      open={open}
      onClose={onClose}
      title={isEdit ? `Edit — ${template?.name}` : 'New Document Template'}
    >
      {err && (
        <div className="mb-4 px-3 py-2 rounded text-sm" style={{ backgroundColor: '#FFF3F3', color: '#D13212', border: '1px solid #F5C6C0' }}>{err}</div>
      )}
      ...
      <Field label="Template name *">
        <input style={inputStyle} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Standard NDA Agreement" />
      </Field>
      ...
      <div className="flex gap-3 mb-4">
        <div style={{ flex: 1 }}>
          <label className="block mb-1 text-xs font-medium" style={{ color: 'var(--text2)' }}>Type</label>
          <select style={selectStyle} value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}>
            {DOC_TEMPLATE_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
          </select>
        </div>
        <div style={{ flex: 1 }}>
          <label className="block mb-1 text-xs font-medium" style={{ color: 'var(--text2)' }}>Category</label>
          <select style={selectStyle} value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>
      ...
      <div className="mb-4">
        ...
        {!preview ? (
          <>
            <VarChips vars={DOC_TEMPLATE_VARS} onInsert={insertVar} />
            <textarea
              ref={bodyRef}
              value={form.content}
              onChange={handleBodyChange}
              rows={10}
              placeholder="This Agreement is entered into between {{company_name}} and {{client_name}}..."
              style={{ ...inputStyle, height: 'auto', padding: '8px 10px', resize: 'vertical', lineHeight: 1.7, fontSize: '0.85rem' }}
            />
            <p className="mt-1 text-xs" style={{ color: 'var(--text3)' }}>
              Plain text or HTML. For rich formatting + canvas layout, edit in the{' '}
              <a href="/templates" style={{ color: 'var(--blue)' }}>Templates page</a>.
            </p>
          </>
        ) : (
          <HtmlPreview html={form.content} />
...
export default function SettingsPage() {
  ...
  const TABS: { id: SettingsTab; label: string; adminOnly?: boolean; creatorOnly?: boolean }[] = [
    ...
    { id: 'doc-templates',   label: 'Doc Templates',   adminOnly: true },

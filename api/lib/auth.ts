...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    ...
    sendResetPassword: async ({ user, url }) => {
      await sendEmail({
        to: user.email,
        subject: 'Reset your CBOP password',
        html: `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Reset your CBOP password</title>
</head>
        ...
        <table width="480" cellpadding="0" cellspacing="0" style="background:#ffffff;border:1px solid #d5dbdb;border-radius:8px;overflow:hidden;">
          ...
          <tr>
            <td style="padding:32px 40px;">
              <p style="margin:0 0 8px;font-size:18px;font-weight:600;color:#16191f;">Reset your password</p>
              <p style="margin:0 0 28px;font-size:14px;color:#4b5563;line-height:1.6;">Click the button below to set a new password. This link expires in 1 hour.</p>
              <a href="${url}" style="display:inline-block;padding:10px 24px;background:#0073bb;color:#ffffff;text-decoration:none;font-size:14px;font-weight:600;border-radius:6px;">Reset password</a>
